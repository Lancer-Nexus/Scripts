...
For a live LLServer instance, copy `llserver.instance.example.json` to `/etc/lancer-nexus/instances/<instance-id>.json` and replace the placeholder paths and IDs. Map `LLSERVER_RUNTIME_STATUS_FILE`, `INSTANCE_ID`, `SYSTEM_ID`, `PRIVATE_BIND_ADDRESS` and `GAME_UDP_PORT` to the JSON fields `RuntimeStatusFile`, `InstanceId`, `SystemId` and `InstanceEndpoint`. Set `MaxPlayers` to `PUBLIC_PLAYER_LIMIT`; the Agent `Agent__Instance__MaxPlayers` must use the same value. The runtime-status file is host-local and must be writable by LLServer and readable by the Agent. `LoginUrl` must remain HTTPS.
